...
int hikcam_init(char* ip, char* usr, char* password)
{
    pthread_t hThread;
    ...
    printf("Init RealPlay SUCCESS\n");
    if (pthread_create(&hThread, NULL, Run_RealPlay,NULL)==-1)
    {
...
cv::Mat get_frame()
{
    ...
    g_frameList.push_back(dst);
    pthread_mutex_unlock(&dst_lock);
    return g_frameList.front();
}
int hikcam_ctrl(int action,int isstop,int speed)
{
    return NET_DVR_PTZControlWithSpeed(lRealPlayHandle,action,isstop,speed);
}
int main()
{
    NET_DVR_Init();
